...
bool Terrain::Initialize(ID3D11Device* device, int terrainWidth, int terrainHeight)
{
	...
	m_frequency = m_terrainWidth / 20; //frequency is used when evaluating the height values for the vertices
	m_amplitude = 3.0;
	m_wavelength = 1;
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++)
		{
			...
			//the X and Z coordinates of each vertex on the terrain match up to the position of the vertex in the array 
			m_heightMap[index].x = (float)i;
			m_heightMap[index].y = (float)height; //defaulted to 0
			m_heightMap[index].z = (float)j;

			//... and then use this step to calculate the texture coordinates for this point on the terrain.
...
bool Terrain::CalculateNormals()
{
	...
	for (j = 0; j<m_terrainHeight; j++)
	{
		for (i = 0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].nx = (sum[0] / length); //unit vector
			m_heightMap[index].ny = (sum[1] / length); //unit vector
			m_heightMap[index].nz = (sum[2] / length); //unit vector
		}
	}
...
bool Terrain::InitializeBuffers(ID3D11Device * device )
{
	...
	m_vertexCount = (m_terrainWidth - 1) * (m_terrainHeight - 1) * 6; //Each face is split into two triangles, therefore it has 6 vertices
	...
	// Get a copy of the vertices array
	loadedVertices = new SimpleMath::Vector3[m_vertexCount];
	for (int i = 0; i < m_vertexCount; i++)
		loadedVertices[i] = vertices[i].position;
...
bool Terrain::GenerateHeightMap(ID3D11Device* device)
{
	...
	//Initialise corners
	m_heightMap[0].y = ((float)rand() / (RAND_MAX)) * m_amplitude;
	m_heightMap[m_terrainHeight * (m_terrainHeight - 1)].y = ((float)rand() / (RAND_MAX)) * m_amplitude;
	m_heightMap[m_terrainWidth - 1].y = ((float)rand() / (RAND_MAX)) * m_amplitude;
	m_heightMap[m_terrainHeight * (m_terrainHeight - 1) + m_terrainWidth - 1].y = ((float)rand() / (RAND_MAX)) * m_amplitude;
