...
bool CollisionDetection::SAT(Player* player, ModelClass rect)
{
	...
	minX2 = rect.GetCentre().x - (rect.GetWidth() / 2.0f);
	maxX2 = minX2 + rect.GetWidth();
	minY2 = rect.GetCentre().y - (rect.GetHeight() / 2.0f);
	maxY2 = minY2 + rect.GetHeight();
	minZ2 = rect.GetCentre().z - (rect.GetDepth() / 2.0f);
